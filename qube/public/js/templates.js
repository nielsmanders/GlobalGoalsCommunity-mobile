...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_sortablejs__ = __webpack_require__(17);
...
$(document).ready(function () {
    ...
    var temp_section = parseInt($(".section-count").html()) + 1;
    var section_count = temp_section - 1;
    var temp_input = parseInt($(".field").not('#ghost').length) + 1;

    // Remove errors
    $(document).on("keydown", "#create-template :input, #edit-template :input", function () {
    ...
    $(document).on("click", ".delete-btn-s", function () {
        ...
        if (button.data('id')) {
            ...
            $.fancyConfirm({
                ...
                message: "Deleting this section will be permanent!",
                okButton: "Delete",
                noButton: "Cancel",
                callback: function callback(value) {
                    if (value) {
                        $.ajax({
                            ...
                            section_count--;
                            $(".section-count").html(section_count);
                        }).catch(function (error) {
                            $(".error-msg-template").html(error.responseText).fadeIn(1000);
                        });
                    }
                }
            });
    ...
    $("#add-section-btn").on("click", function () {
        var sections = $(".section").not('.ghost-section');
        var error = false;

        // Check if last section has at least 1 field
        if (sections.length > 0) {
            if (sections.last().find("input").length < 2) {
                error = true;
            }
        }

        // If there is no error add new section
        if (!error) {
            var section = $(".ghost-section").clone();
            section.removeClass("ghost-section hidden");
            section.find(".error-msg").addClass("error-msg-" + temp_section);
            section.find(".label").attr("for", "section_" + temp_section + "_name");
            section.find("input").attr({
                id: "section_" + temp_section + "_name",
                name: "section_" + temp_section + "_name",
                "data-section": temp_section
            });
            section.attr("data-section", temp_section);

            $("#create-template, #edit-template").find(".sections").append(section);
            ...
            $(".error-msg-" + (temp_section - 1)).html("Add at least one field to the section").fadeIn(1000);
        }
    });
    ...
    $(document).on("click", ".delete-btn-f", function () {
        ...
        if (button.data('id')) {
            ...
            $.fancyConfirm({
                ...
                message: "Deleting this field will be permanent!",
                okButton: "Delete",
                noButton: "Cancel",
                callback: function callback(value) {
                    if (value) {
                        $.ajax({
                            ...
                            dataType: "json",
                            data: {
                                action: 'delete-field-template',
                                field_id: field_id
                            }
                        }).done(function () {
                            button.closest(".input-wrapper").remove();
                        }).catch(function (error) {
                            $(".error-msg-template").html(error.responseText).fadeIn(1000);
                        });
                    }
    ...
    $("[data-action='add-field']").on("click", function () {
        if ($("#create-template, #edit-template").find(".section").length === 1) {
            $(".error-msg-template").html("Please add at least one section").fadeIn(1000);
        } else {
            var field_id = $(this).data("id");
            var field_name = $(this).data("name");
            field_name = field_name.charAt(0).toUpperCase() + field_name.slice(1);

            var input = $(".ghost-input").clone();
            input.removeClass("ghost-input hidden");
            input.find(".label").attr("for", "input_" + temp_input + "_name").html(field_name + " name");
            input.find("input").attr({
                id: "input_" + temp_input + "_name",
                name: "input_" + temp_input + "_name",
                "data-field-id": field_id,
                "data-input": temp_input
            });

            $(".section").last().find(".fields").append(input);
    ...
    $(document).on("click", "#create-template-btn", function () {
        var form = $("#create-template");

        var values = $.grep(form.serializeArray(), function (e) {
            return e.name != "ghost";
        });
        var emptyValues = [];
        ...
        if (emptyValues.length > 0) {
            $.each(emptyValues, function (index, el) {
                if (el.includes("template")) {
                    $(".error-msg-template").html("Please fill in the required fields").fadeIn(1000);
                } else {
                    if (el.includes("input")) {
                        $("#" + el).parent().parent().parent().find(".error-msg").html("Please fill in the required fields").fadeIn(1000);
                    } else {
                        $("#" + el).parent().parent().find(".error-msg").html("Please fill in the required fields").fadeIn(1000);
                    }
                }
            ...
            // Check if template has at least 1 section
            if (form.find(".section").length === 1) {
                $(".error-msg-template").html("Please add at least one section").fadeIn(1000);
            } else {
                // Reset error to false
                var error = false;

                // Check if template has at least 1 input field
                if (form.find(".field").length === 1) {
                    $(".error-msg-" + (temp_section - 1)).html("Please add at least one field").fadeIn(1000);
                }

                // Check if each section has at least one field
                var sections = $(".section").not('.ghost-section');
                $.each(sections, function (index, el) {
                    if ($(el).find('.field').length === 0) {
                        error = true;
                        $(el).find('.error-msg').html("Please add at least one field").fadeIn(1000);
                    }
                });

                if (!error) {
                    var sectionsObject = [];
                    var _sections = $(".section_input:not(#ghost)");

                    $.each(_sections, function (index, el) {
                        var sectionNr = $(el).attr('data-section');

                        var fieldsObject = [];
                        var fields = $(".section[data-section = '" + sectionNr + "']").find(".field");
                        $.each(fields, function (index, el) {
                            var field = {
                                value: $(el).val(),
                                number: $(el).attr('data-input'),
                    ...
                    $.ajax({
                        ...
                        if (!response["success"]) {
                            if (response['message']) {
                                $(".error-msg-template").html(response["message"]).fadeIn(1000);
                            } else {
                                $(".error-msg-" + response['error']['section']).html(response['error']['message']).fadeIn(1000);

                                if (response['error']['type'] === "section") {
                                    $("#section_" + response['error']['section'] + "_name").addClass("error");
                                } else if (response['error']['type'] === "field") {
                                    $("#input_" + response['error']['field'] + "_name").addClass("error");
                                }
                            }
                        } else {
    ...
    $(document).on("click", "#edit-template-btn", function () {
        ...
        if (emptyValues.length > 0) {
            ...
            if (form.find(".section").length === 1) {
                ...
                // If there are no errors, setup the json
                if (!error) {
                    var sectionsObject = [];
                    var _sections2 = $(".section_input:not(#ghost)");

                    $.each(_sections2, function (index, el) {
                        var sectionNr = $(el).attr('data-section');
                        ...
                        var section = {
                            section_id: $(el).attr('data-section-id'),
                            value: $(el).val(),
                            number: sectionNr,
                            fields: fieldsObject
                        };
...
// Sortable sections
if ($(".sections").length) {
    new __WEBPACK_IMPORTED_MODULE_0_sortablejs__["a" /* default */]($(".sections")[0], {
        animation: 150,
        swapThreshold: 1,
        handle: '.sort-handle-s'
    });
}
...
function refreshSortable() {
    ...
    $.each(sortableFields, function (index, el) {
        new __WEBPACK_IMPORTED_MODULE_0_sortablejs__["a" /* default */](el, {
            animation: 150,
            swapThreshold: 1,
            handle: '.sort-handle-f',
            group: 'fields'
        });
    });
}
